...
(define threewayswitch%
  (class switch%
    (super-new)
    (init-field first-switch-id second-switch-id)
    (inherit-field id switch-track)
    (define/public (update-switch first-switch-value second-switch-value)
      (if (= first-switch-value 0)
          (send this set-switch-position 0)
          (if (= second-switch-value 0)
              (send this set-switch-position 1)
              (send this set-switch-position 2))))))
